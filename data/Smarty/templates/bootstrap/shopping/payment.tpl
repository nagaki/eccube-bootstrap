  ...
  <div id="undercolumn_shopping">
    ...
    <form name="form1" id="form1" method="post" action="?">
      ...
      <div class="pay_area">
        ...
        <!--{if $arrErr[$key] != ""}-->
        <p class="alert alert-warning"><!--{$arrErr[$key]}--></p>
        <!--{/if}-->
        <table class="table">
          <colgroup>
            <col width="20%" />
      ...
      <div class="pay_area">
        ...
        <table class="table" summary="お支払方法選択" id="payment">
          <colgroup>
            <col width="20%" />
            <col width="80%" />
          </colgroup>
      ...
      <div class="pay_area02">
        ...
        <div class="delivdate top">
          ...
          <div class="form-group">
            ...
            <label>お届け日</label>
            <!--{if !$arrDelivDate}-->
            <p class="help-block">ご指定頂けません。</p>
            <!--{else}-->
            <select class="form-control" name="<!--{$key}-->" id="<!--{$key}-->" style="<!--{$arrErr[$key]|sfGetErrorColor}-->">
            ...
            <!--{/if}-->
            <span class="alert alert-warning"><!--{$arrErr[$key]}--></span>
          </div>
          <!--★お届け時間★-->
          <div class="form-group">
      ...
      <div class="point_area">
        ...
        <div class="point_announce">
          <p><span class="user_name"><!--{$name01|h}--> <!--{$name02|h}-->様</span>の、現在の所持ポイントは「<span class="point"><!--{$tpl_user_point|default:0|number_format}-->ポイント</span>」です。<br />
            今回ご購入合計金額：<span class="price"><!--{$arrPrices.subtotal|number_format}-->円</span> <span class="attention">(送料、手数料を含みません。)</span>
          </p>
          <div class="radio">
            <label for="point_on">
              <input type="radio" id="point_on" name="point_check" value="1" <!--{$arrForm.point_check.value|sfGetChecked:1}--> onclick="eccube.togglePointForm();" />
              ポイントを使用する
            </label>
          </div>
          <div class="form-group">
            <!--{assign var=key value="use_point"}-->
            <label>今回のお買い物で使用するポイント</label>
            <input class="form-control" type="text" name="<!--{$key}-->" value="<!--{$arrForm[$key].value|default:$tpl_user_point}-->" maxlength="<!--{$arrForm[$key].length}-->" style="<!--{$arrErr[$key]|sfGetErrorColor}-->" />
            <span class="halp-block"><!--{$arrErr[$key]}--></span>
          </div>
          <div class="radio">
            <label for="point_off">
      ...
      <div class="pay_area02">
        ...
        <div>
          ...
          <textarea class="form-control" name="<!--{$key}-->" style="<!--{$arrErr[$key]|sfGetErrorColor}-->" cols="70" rows="8" class="txtarea" wrap="hard"><!--{"\n"}--><!--{$arrForm[$key].value|h}--></textarea>
          <p class="attention"> (<!--{$smarty.const.LTEXT_LEN}-->文字まで)</p>
        </div>
      </div>

      <div class="btn_area">
        <a class="btn btn-default" href="?mode=return" name="back03" id="back03">戻る</a>
