        ...
        <div class="cart_area clearfix">
          ...
          <!--{if $tpl_stock_find}-->
            ...
            <div class="form-group <!--{if $arrErr.quantity != ""}-->has-error<!--{/if}-->">
              <label>数量</label>
              <input type="text" class="form-control" name="quantity" value="<!--{$arrForm.quantity.value|default:1|h}-->" maxlength="<!--{$smarty.const.INT_LEN}-->" style="<!--{$arrErr.quantity|sfGetErrorColor}-->" />
              <!--{if $arrErr.quantity != ""}-->
            ...
            <!--★カゴに入れる★-->
            <a class="btn btn-default" href="javascript:void(document.form1.submit())">
              カゴに入れる
            </a>
          ...
          <div class="form-group <!--{if $arrErr[$add_favorite]}-->has-error<!--{/if}-->">
            <!--{assign var=add_favorite value="add_favorite`$product_id`"}-->
            <!--{if !$is_favorite}-->
              <a class="btn btn-default" href="javascript:eccube.changeAction('?product_id=<!--{$arrProduct.product_id|h}-->'); eccube.setModeAndSubmit('add_favorite','favorite_product_id','<!--{$arrProduct.product_id|h}-->');">お気に入りに追加</a>
            <!--{else}-->
              お気に入り登録済
            <!--{/if}-->
            <!--{if $arrErr[$add_favorite]}-->
              <span class="help-block"><!--{$arrErr[$add_favorite]}--></span>
            <!--{/if}-->
          </div>
          <!--{/if}-->
        </div>
  ...
  <div id="customervoice_area">
    ...
    <div class="review_bloc">
      ...
      <div class="review_btn">
        ...
        <!--★新規コメントを書き込む★-->
        <a class="btn btn-default" href="./review.php"
